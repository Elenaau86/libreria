"""
ingesta_lineas_incremental.py
Script de ingesta INCREMENTAL de LINEAS_VENTA_RAW desde CSV a Bronze.

Misma lógica que ingesta_ventas_incremental.py:
- NO hace TRUNCATE. Append puro.
- Espera ficheros con nombre LINEAS_VENTA_YYYYMMDD.csv.
- Snowflake usa LOAD_HISTORY para evitar reprocesar ficheros ya cargados.
...
Uso:
    ...
    python ingesta_lineas_incremental.py --fichero LINEAS_VENTA_20250512.csv --entorno PRE
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingesta incremental de LINEAS_VENTA_RAW en Bronze."
    )
    parser.add_argument(
        "--fichero",
        required=True,
        help="Nombre del CSV a cargar, p.ej. LINEAS_VENTA_20250512.csv",
    )
    parser.add_argument(
        "--entorno",
        choices=["DEV", "PRE", "PRO"],
...
def main() -> None:
    ...
    print(f"{'─'*60}")
    print(f"Ingesta incremental LINEAS_VENTA")
    ...
    try:
        ...
        verificar(cursor)
        print(f"\n{'─'*60}")
        print("Ingesta incremental de líneas completada.")
